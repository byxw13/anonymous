...
class AdvancedSelfConsistency:
    ...
    def extract_final_option(self, text: str) -> str:
        pattern = r"option\s*([A-E])"
        match = re.search(pattern, text, re.IGNORECASE)
        if match:
            return match.group(1).upper()
        return None

    def calculate_similarity(self, a: str, b: str,evaluator) -> float:
        a = evaluator.extract_answer(a)
        b = evaluator.extract_answer(b)
        if not a :
            a = evaluator.extract_final_answer(a)
        if not b:
            b = evaluator.extract_final_answer(b)
        option_a = a
        option_b = b
...
def process_question_with_self_consistency(entry, evaluator, model):
    global True_count
    try:
        ...
        messages = [
            {"role": "user",
             "content": f"Here is a multiple-choice question about mathematics. Please reason through it step by step, and at the end, provide your answer option with 'the answer is  X', Where 'X' is the correct option you think from A,B,C,D,E. Here is the question you need to answer:\n{full_question}\nLet's think step by step:"}
        ]
        ...
        expected_number = f"{answer.upper()}. {selected_option}"
        ...
        while try_num < max_try:
            if try_num == 0:
                ...
                evaluator.token_statistics = evaluator.token_statistics.append({
                    ...
                    "Experiment": "Self-Consistency+NiCOT",
...
def process_directory(directory):
    global True_count
    input_file = f"{directory}/test.json"
    ...
    while count < 1: 
        for i in range(6):  
            ...
            with ThreadPoolExecutor() as executor:
                results = list(executor.map(lambda entry: process_question_with_self_consistency(entry, evaluator, MODEL_NAME), data[n:m]))
            

            output_file = f"Aqua_self_consistency+Reverse_{directory}_{MODEL_NAME}_aqua_results_{n}-{m}.json"
            ...
            evaluator.save_statistics(filename=f"Aqua_self_consistency+Reverse_{directory}_{MODEL_NAME}_statistics_{n}-{m}.csv")
...
print("Self-consistency evaluation for aqua dataset complete.")