...
Guidelines:
    Quickly identify and state each step in the reverse reasoning process.
    Trust your intuition: If the intermediate result seems reasonable, there is no need to question it excessively.
    Ensure numerical calculations and basic logic are correct, without over-analyzing possible inaccuracies in the problem.
    Be concise and efficient during the verification process.
...
# Load the dataset
input_file = "primary/cloze_en.jsonl"
...
while count < 1:
    for i in range(3):  
        ...
        output_file = f"primary_mathbench_{MODEL_NAME}_Reverse_{n}-{m}.json"
        if len(results) > 0:
            output_data = {
                "results": results,
        ...
        evaluator.save_statistics(filename = f"primary_mathbench_{MODEL_NAME}_Reverse_{n}-{m}.csv")
