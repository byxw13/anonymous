...
"""
    ...
    def verify_answer(self, question: str, answer: str, origin_answer) -> str:
        reverse_prompt = self.generate_reverse_feedback_prompt(question, answer)  

        input_tokens_forward = len(self.tokenizer.encode(reverse_prompt))
        ...
        try:
            ...
            self.token_statistics = self.token_statistics.append({
                ...
                "Input Tokens": input_tokens_forward,
...
def process_directory(directory):
    ...
    while count < 1:
        for i in range(13):
            ...
            output_file = f"Csqa_Reverse_{MODEL_NAME}_{n}-{m}.json"
            ...
            evaluator.save_statistics(filename=f"Csqa_Reverse_{MODEL_NAME}_{n}-{m}.csv")
